...
gg <- ggplot(toplot, aes(month,pct, group=cat, color=cat, label=cat)) +
  ...
  scale_y_continuous("Percent of Monthly Published CVEs", label=label_percent(1), expand=c(0,0))
savepic(gg, here::here("figs", params$img_slug, "cves_published_with_cvss_version"), 
...
gg <- ggplot(toplot, aes(month,pct, group=cat, color=cat, label=cat)) +
  ...
  theme(legend.position = "none") +
  scale_y_continuous("Percent of Monthly Published CVEs", label=label_percent(1), expand=c(0,0))
savepic(gg, here::here("figs", params$img_slug, "cves_published_with_cvss_version_no_mitre"), 
        height = 3, width = 7, open = FALSE, caption=TRUE, dpi=300) %>%
  knitr::include_graphics()

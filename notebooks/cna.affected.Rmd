...
gtable <- toshare %>% 
  ...
  gt() |> 
    tab_header(
    title = "CNA containers for \"affected\"",
    subtitle = paste(cves %>% distinct(cve) %>% nrow() %>% comma, 
  ...
  # cves %>% distinct(cve) %>% nrow

  if (!dir.exists(here("figs", params$img_slug))) {
    dir.create(here("figs", params$img_slug), recursive = TRUE, mode="0755")
  }
  rez <- gtsave(gtable, here::here("figs", params$img_slug, "affected_counts_table.png"),
                vwidth = 992, vheight=544)
...
savepic(gg, here::here("figs", params$img_slug, "filename"), 
        height = 3, width = 5, open = FALSE, caption=FALSE, dpi=300) %>%
  knitr::include_graphics()
```

```{r}
if(!file.exists(here("cache", "cpe_dict_2.3.parquet"))) {
  if(!file.exists(here("data", "official-cpe-dictionary_v2.3.xml"))) {
    cpefile <- "https://nvd.nist.gov/feeds/xml/cpe/dictionary/official-cpe-dictionary_v2.3.xml.gz"
    download.file(cpefile, destfile = here("data", basename(cpefile)))
  }
  # may take some time, but trust me, reading and parsing raw xml is easier than loading the xml object.
  cpedict <- tibble(x=read_lines(here("data", "official-cpe-dictionary_v2.3.xml.gz"))) %>% 
...
gg <- ggplot(toplot, aes(x=pct, y=match, fill=match, label=txt)) +
  ...
  theme(axis.text.x = element_blank(), 
        ...
        axis.title.y = element_blank())
savepic(gg, here::here("figs", params$img_slug, "cpe_matching_to_CVEs"), 
...
savepic(gg, here::here("figs", params$img_slug, "monpct.prod_vend_version"), 
...
savepic(gg, here::here("figs", params$img_slug, "vpv_complete_by_cna"), 
...
gg <- ggplot(url_pub, aes(month, pct, color=colfill)) + 
  ...
  theme(plot.title = element_text(family=mainfont, face="italic", size=11))
savepic(gg, here::here("figs", params$img_slug, "monpct.containers.cna.affected.collectionURL"), 
        height = 3.5, width = 7, open = FALSE, caption=TRUE, dpi=300) %>%
  knitr::include_graphics()
```
...
savepic(gg, here::here("figs", params$img_slug, "cnapct.affected.collectionURL"), 
...
savepic(gg, here::here("figs", params$img_slug, "cnapct2020.affected.collectionURL"), 
        height = 4, width = 7, open = FALSE, caption=TRUE, dpi=300) %>%
  knitr::include_graphics()
...
gg <- ggplot(toplot, aes(n, pct, label=cna)) +
  ...
  theme(panel.grid = element_line(color="gray95"),
        plot.title = element_text(family=mainfont, face="italic", size=11))
savepic(gg, here::here("figs", params$img_slug, "cna_uniq.affected.collectionURL"), 
        height = 3.5, width = 7, open = FALSE, caption=TRUE, dpi=300) %>%
  knitr::include_graphics()
...
gg <- ggplot(url_pub, aes(month, pct, color=colfill)) + 
  ...
  labs(title = paste("Percent of Published CVEs with values in", curfield)) +
  theme(plot.title = element_text(family=mainfont, face="italic", size=11))
savepic(gg, here::here("figs", params$img_slug, "monpct.affected.cpes"), 
        height = 3.5, width = 7, open = FALSE, caption=TRUE, dpi=300) %>%
  knitr::include_graphics()
...
savepic(gg, here::here("figs", params$img_slug, 
                       paste0("cnapct_", gsub("containers.cna.", "", curfield))), 
        height = 4, width = 7, open = FALSE, caption=TRUE, dpi=300) %>%
  knitr::include_graphics()
```
